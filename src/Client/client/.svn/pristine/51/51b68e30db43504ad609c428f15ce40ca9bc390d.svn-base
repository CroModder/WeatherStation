...
public class HistoricalController {
        ...
        historicalTabPane.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			...
			public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
				System.out.println("\nnew value " + newValue + "old value " + oldValue);
					...
					if(tabIndex == 0){
						radioSelection = "rbCelsius";
						lineChartAcc.getData().clear();
						lineChartAir.getData().clear();
						lineChartMag.getData().clear();
					}else if(tabIndex == 1){
    ...
    private void datePickerEnd(){
    	try {
    		...
    		beginDate = new SimpleDateFormat("dd.MM.yyyy").format(resBegin);
    		endDate = new SimpleDateFormat("dd.MM.yyyy").format(resEnd);
        	Calendar cal = Calendar.getInstance();
        	cal.setTime(resEnd);
        	System.out.println(endDate.toString().equals(dateFormat.format(cal.getTime())));
        	if(endDate.toString().equals(beginDate)){
        		cal.set(Calendar.HOUR_OF_DAY, 23);
        		cal.set(Calendar.MINUTE, 59);
        		endDate = dateFormatDB.format(cal.getTime());
        		System.out.println("jednako" + dateFormatDB.format(cal.getTime()));
        	}
    	...
    	if(event.getSource().equals(rbCelsius)){
    		...
    		for(int i=0;i<lineChart.getData().size();i++){
    			...
    			for(int j=0;j<lineChart.getData().get(i).getData().size();j++){
    				series.getData().add(new XYChart.Data<String, Number>(lineChart.getData().get(i).getData().get(j).getXValue().toString(),(lineChart.getData().get(i).getData().get(j).getYValue().doubleValue() - 32) * 0.55555555555));
    			}
    			elements.add(series); 
    			series = new LineChart.Series<String, Number>();
    			series.getData().clear();
    		...
    		for(int i=0;i<lineChart.getData().size();i++){
    			nameSeries[i] = lineChart.getData().get(i).getName();
    			for(int j=0;j<lineChart.getData().get(i).getData().size();j++){
    				series.getData().add(new XYChart.Data<String, Number>(lineChart.getData().get(i).getData().get(j).getXValue().toString(),(lineChart.getData().get(i).getData().get(j).getYValue().doubleValue()*1.8) + 32));
    			}
    			elements.add(series); 
    			series = new LineChart.Series<String, Number>();
    			series.getData().clear();
    ...
    @SuppressWarnings("unchecked")
	public void setDateAxis(LineChart.Series<String, Number> series, String name, Integer idTable){
        List<String> dates = new ArrayList<String>();
    	if(idTable == 0){
    		for(int i=0;i<series.getData().size();i++){
    			dates.add(series.getData().get(i).getXValue());
    		}
    		System.out.println("conatins" + lineChart.getData().size());
        		datexAxsis.clear();
    		...
    		dateAxisAir.setTickLabelRotation(90);
    		dateAxisAir.setAutoRanging(false);
    		NumberAxis nuberAxis = new NumberAxis();
        	...
        	series.setName(name);
    		lineChartAir.getData().addAll(series);
    	}else if(idTable == 2){
    		for(int i=0;i<series.getData().size();i++){
    			dates.add(series.getData().get(i).getXValue());
    		}
    		...
    		dateAxisMag.setTickLabelRotation(90);
    		dateAxisMag.setAutoRanging(false);
    		NumberAxis nuberAxis = new NumberAxis();
        	...
        	series.setName(name);
    		lineChartMag.getData().addAll(series);
    	}else{
